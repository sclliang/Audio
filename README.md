...
```powershell
# 1：切换扬声器（可用 next、序号、完整设备 ID 或唯一的名称片段）
audioctl 1 next
audioctl 1 1
...
设备切换会同时修改 Windows 的控制台、多媒体和通信三种默认角色。动作 `1` 和 `2` 的设备参数可以写 `next`，表示切换到当前默认设备的下一个活动设备；如果当前默认设备已经是最后一个设备，则自动切换到序号 `0`。名称匹配不区分大小写；名称匹配到多个设备时，程序会要求改用序号或完整设备 ID。
